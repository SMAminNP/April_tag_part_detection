...
class ObjectDetectorROS:
    def __init__(self):
        ...
        self.model_path = '/home/amin/YOLO/runs/detect/train/weights/bestC.pt' 
        self.threshold = 0.5
        ...
        rospy.Subscriber('/rs_camera_1/color/image_rect', Image, self.color_image_callback) 
        rospy.Subscriber('/rs_camera_1/depth/registered/image_rect_raw', Image, self.depth_image_callback) 
        rospy.Subscriber('/rs_camera_1/depth/camera_info', CameraInfo, self.camera_info_callback) 

        self.object_coordinates_pub = rospy.Publisher('/object_coordinates', ObjectCoordinates, queue_size=10)
        self.annotated_image_pub = rospy.Publisher('/Sensor', Image, queue_size=10)  
            ...
            if self.color_image is not None and self.depth_image is not None and self.camera_info is not None:
                ...
                for result in results.boxes.data.tolist():
                    ...
                    if score > self.threshold:
                        ...
                        depth = self.depth_image[y_center, x_center] * 0.013  
                        ...
                        cv2.rectangle(self.color_image, (int(x1), int(y1)), (int(x2), int(y2)), (0, 255, 0), 2)
                        label = f"{results.names[int(class_id)]}: {score:.2f}"
                        cv2.putText(self.color_image, label, (int(x1), int(y1) - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
                ...
                #cv2.imshow("Color Image with Detections", self.color_image)
                #cv2.waitKey(1)
